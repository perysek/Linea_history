...
def _parse_date_param(date_str, default=None):
    """Validate YYYY-MM-DD date string from request args. Returns validated string or default."""
    if date_str:
        try:
...
def _to_mosys_date(date_str):
    """Convert YYYY-MM-DD to YYYYMMDD for MOSYS SQL."""
    return date_str.replace('-', '') if date_str else None


def _build_nc_details_json(matched_ncs):
    """Shared helper: enrich a list of NC dicts with blocked qty + sorting data, return JSON-ready list."""
    ...
    if nr_list:
        try:
            ...
            q_blocked = f'''
                ...
                WHERE SEGCONF.NUMERO_NON_CONF IN ({ph})
            ...
            for _, row in df_blocked.iterrows():
                blocked_map[str(row['NUMERO_NON_CONF']).strip()] = int(row['TOTAL']) if row['TOTAL'] else 0
        except Exception as e:
            print(f"[_build_nc_details_json] blocked qty error: {e}")

    # Sorting summary from SQLite
    sorting_map = {}
    if nr_list:
        try:
            reports = DaneRaportu.query.options(
            ...
            print(f"[_build_nc_details_json] SQLite sorting error: {e}")
    ...
    # Open NCs first, then by nr desc (stable two-step sort)
    ncs_out.sort(key=lambda x: x['nr_niezgodnosci'], reverse=True)
    ncs_out.sort(key=lambda x: x['is_open'], reverse=True)
    return ncs_out
...
def analiza_danych():
    """Analiza danych — top NC part codes (CODICE_ARTICOLO) in a date range."""
    from datetime import date, timedelta
    from collections import Counter
    from MOSYS_data_functions import get_all_notcojan_for_analysis

    today = date.today()
    default_from = (today - timedelta(days=6)).isoformat()
    default_to = today.isoformat()

    date_from = _parse_date_param(request.args.get('date_from'), default_from)
    date_to = _parse_date_param(request.args.get('date_to'), default_to)
    ...
    try:
        all_ncs = get_all_notcojan_for_analysis(
            date_from=_to_mosys_date(date_from),
            date_to=_to_mosys_date(date_to),
        )
        total_nc_count = len(all_ncs)
        counts = Counter(
            (nc.get('codice_articolo') or '—').strip() or '—'
            for nc in all_ncs
        )
        top10 = [{'name': art, 'count': cnt} for art, cnt in counts.most_common(10)]
    except Exception as e:
        error_msg = str(e)
...
@placeholder_bp.route('/api/analiza-top10')
@module_required('analiza')
def api_analiza_top10():
    """AJAX: top 10 CODICE_ARTICOLO by NC count for a given date range."""
    from datetime import date, timedelta
    from collections import Counter
    from MOSYS_data_functions import get_all_notcojan_for_analysis

    today = date.today()
    date_from = _parse_date_param(request.args.get('date_from'), (today - timedelta(days=6)).isoformat())
    date_to = _parse_date_param(request.args.get('date_to'), today.isoformat())
...
@placeholder_bp.route('/api/nc-article-details/<path:codice_articolo>')
@module_required('analiza')
def api_nc_article_details(codice_articolo):
    """AJAX: per-NC details for a given CODICE_ARTICOLO in a date range."""
    from datetime import date, timedelta
    from MOSYS_data_functions import get_all_notcojan_for_analysis
    ...
    try:
        ...
        matched_ncs = [
            nc for nc in all_ncs
            if ((nc.get('codice_articolo') or '').strip() or '—') == codice_articolo
        ]
        ncs_out = _build_nc_details_json(matched_ncs)
        return jsonify({
            'success': True,
        ...
        print(f"[api_nc_article_details] error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
