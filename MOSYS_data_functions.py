...
# ─── NC Category Configuration ─────────────────────────────────────────────
# Edit this list to define your keyword categories.
# Each entry: name (display), color (hex), pattern (Python regex, case-insensitive).
# The FIRST matching category wins; 'Inne' must be the last entry (catch-all).
NC_KEYWORD_CATEGORIES = [
    {'name': 'Niedolanie',       'color': '#ef4444', 'pattern': r'niedola|dziur'},
    {'name': 'Przelanie',        'color': '#f97316', 'pattern': r'przelan|zalan|nadlew'},
    {'name': 'Wypływka',         'color': '#3b82f6', 'pattern': r'wyplywk'},
    {'name': 'Uszkodzenie',      'color': '#8b5cf6', 'pattern': r'uszkodz|rysa|wylaman|ulaman'},
	{'name': 'Przypalenie',      'color': '#8b5cf6', 'pattern': r'przypal|nadpal'},
	{'name': 'Wysoki punkt wtrysku',      'color': '#8b5cf6', 'pattern': r'punkt|wysok|wystajac'},
    {'name': 'Deformacja',       'color': '#f91316', 'pattern': r'zdeform|deform|wygiet|krzyw'},  # catch-all — keep last
	{'name': 'Inne',             'color': '#6b7280', 'pattern': r'.'},
]
...
def get_all_notcojan_for_analysis(date_from: str = None, date_to: str = None) -> list:
    ...
    try:
        ...
        # Aggregate notes per NUMERO_NC — combine all rows for the same NC
        nc_map = {}
        for _, row in df.iterrows():
            nr = str(row['NUMERO_NC']).strip() if row['NUMERO_NC'] else None
            if not nr:
                continue

            # Collect note fragments from this row
            note_parts = []
            for i in range(1, 11):
                note = None
                for col_name in [f'NOTE_{i:02d}', f'NOTE{i:02d}', f'NOTE_{i}', f'NOTE{i}']:
            ...
            if nr not in nc_map:
                nc_map[nr] = {
                    'nr_niezgodnosci': nr,
                    'commessa': str(row['COMMESSA']).strip() if row['COMMESSA'] else '',
                    'data_nc': parse_mosys_date(row['DATA']),
                    'notes_parts': [],
                }
        ...
        print(f"Error in get_all_notcojan_for_analysis: {e}")
        return []
